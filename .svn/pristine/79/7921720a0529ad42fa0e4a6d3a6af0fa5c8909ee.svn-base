...
!
function(mui, zs) {
	zs.SurroundingDynamicDynamicPage = zs.UserPage.extend({
		...
		initPage: function() {
			...
			mui.plusReady(function() {
				...
				//从本机取动态缓存
				var surroundingDynamicDynamic = zs.getItem('surroundingDynamicDynamic');
				try {
					surroundingDynamicDynamic = JSON.parse(surroundingDynamicDynamic);
				} catch(e) {
					//TODO handle the exception
					surroundingDynamicDynamic = false;
				}
				surroundingDynamicDynamic && cls.renderDynamic(surroundingDynamicDynamic, 1);
				pulldownRefresh();
				mui.init({
					pullRefresh: {
				...
				function pulldownRefresh() {
					...
					setTimeout(function() {
						...
						params.type = 'other',
							params.pagesize = 10;
						params.pagenum = 1;
						zs.Api.post('say', 'lists', params, function(surroundingDynamicDynamic) {
							//处理返回值 de 时间格式 图片地址  距离 另外处理
							var data = surroundingDynamicDynamic.data;
							...
							//先更新缓存，再调用填充函数
							zs.setItem('surroundingDynamicDynamic', JSON.stringify(surroundingDynamicDynamic));
							$('#mui-content').empty();
							cls.renderDynamic(surroundingDynamicDynamic, 1);
				...
				function pullupRefresh() {
					setTimeout(function() {
						...
						zs.Api.post('say', 'lists', params, function(surroundingDynamicDynamic) {
							...
							var len = $('.simplify-dynamic-box').length;
							...
							//先更新缓存，再调用填充函数
							//从本机取动态缓存
							var oldsurroundingDynamicDynamic = zs.getItem('surroundingDynamicDynamic');
							try {
								oldsurroundingDynamicDynamic = JSON.parse(oldsurroundingDynamicDynamic);
								var olddata = oldsurroundingDynamicDynamic.data;
								for (i=0;i<data.length;i++){
									olddata.push(data[i]);								
								}	
								zs.setItem('surroundingDynamicDynamic', JSON.stringify(oldsurroundingDynamicDynamic));
							} catch(e) {
								//TODO handle the exception
								zs.setItem('surroundingDynamicDynamic', JSON.stringify(surroundingDynamicDynamic));
							}
							
							cls.renderDynamic(surroundingDynamicDynamic, 0);
		...
		initEvent: function() {
			...
			window.addEventListener('addPinglun', function(event) {
				...
				var surroundingDynamicDynamic = JSON.parse(zs.getItem('surroundingDynamicDynamic'));
				var thisDynamic = surroundingDynamicDynamic.data[dynamicIndex];
				//console.log(JSON.stringify(thisDynamic));
		...
		//动态的填充函数
		renderDynamic: function(surroundingDynamicDynamic, type) {
			var cls = this;
			data = surroundingDynamicDynamic.data;
			zs.template('mui-content', 'surrounding_dynamic_dynamic', {
				list: data
			}, 'append');
		...
		clickEvent: function() {
			...
			mui.plusReady(function() {
				...
				mui(document.body).on('tap', 'i[data-dianzan]', function(event) {
						...
						cls.val = $(_this).attr('data-val');
						cls.dynamicIndex = parseInt(cls.dynamicIndex) + parseInt(cls.val);
						cls.thisDynamic = surroundingDynamicDynamic.data[cls.dynamicIndex];
						...
						if(isbusy) {
							...
							if($(_this).hasClass('dianzan-dianji-icon-active')) {
								zs.Api.post('say_like', 'delete', params, function(r) {
									...
									for(var i = 0; i < cls.thisDynamic.say_likeInfo.length; i++) {
										if(params.uid == cls.thisDynamic.say_likeInfo[i].uid) {
											...
											//console.log(JSON.stringify(cls.thisDynamic.say_likeInfo));
									...
									//修改并保存缓存
									surroundingDynamicDynamic.data[cls.dynamicIndex] = cls.thisDynamic;
									zs.setItem('surroundingDynamicDynamic', JSON.stringify(surroundingDynamicDynamic));
								...
								zs.Api.post('say_like', 'add', params, function(r) {
									...
									if(isArray(cls.thisDynamic.say_likeInfo)) {} else {
									...
									//存缓存
									surroundingDynamicDynamic.data[cls.dynamicIndex] = cls.thisDynamic;
									var cache = JSON.stringify(surroundingDynamicDynamic);
				...
				mui(document.body).on('tap', 'i[data-pinlun]', function(event) {
					...
					setTimeout(function() {
						...
						//console.log(JSON.stringify(dynamic_detail));
