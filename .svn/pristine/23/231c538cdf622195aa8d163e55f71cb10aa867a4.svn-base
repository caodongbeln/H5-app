...
!
function(mui, zs) {
	zs.MyActivityRefreshPage = zs.UserPage.extend({
		...
		initPage: function() {
			...
			mui.plusReady(function() {
				//从本机取动态缓存
				var myActivity = zs.getItem('myActivity');
				zs.d(myActivity);
				try {
					myActivity = JSON.parse(myActivity);
				} catch(e) {
					//TODO handle the exception
					myActivity = false;
				}

				myActivity && cls.renderDynamic(myActivity, 1);
				...
				function pulldownRefresh() {
					...
					setTimeout(function() {
						...
						params.pagesize = 10;
						params.pagenum = 1;
						zs.Api.post('strategy', 'lists', params, function(myActivity) {
							//处理返回值 de 时间格式 图片地址  距离 另外处理
							var data = myActivity.data;
							if(data == null || data == '') {
								var no_results = "<div>您还没有发布任何活动信息</div>"
							...
							//先更新缓存，再调用填充函数
							zs.setItem('myActivity', JSON.stringify(myActivity));
							$('#mui-content').empty();
							cls.renderDynamic(myActivity, 1);
							zs.closeWaiting();
						});
				...
				function pullupRefresh() {
					setTimeout(function() {
						...
						zs.Api.post('strategy', 'lists', params, function(myActivity) {
							...
							cls.renderDynamic(myActivity, 0);
		...
		//动态的填充函数
		renderDynamic: function(myActivity, type) {
			var cls = this;
			data = myActivity.data;
			zs.template('mui-content', 'my_activity', {
				list: data
			}, 'append');
			//alert(tt);
		...
		addDianzan: function(activityid, index) {
			...
			//取该条活动的缓存
			var myActivity = JSON.parse(zs.getItem('myActivity'));
			cls.thisActivity = myActivity.data[index];
			//修改标记是否有人点赞的字段
			...
			$(id).addClass('dianzan-active');
			//存缓存
			myActivity.data[index] = cls.thisActivity;
			var cache = JSON.stringify(myActivity);
